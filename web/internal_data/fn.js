        ...
        if (error) {
            ...
            function changeColor(o) {
                if (o['href'] == href) {
                    ...
                    let collection = new Collection(o['href'], o['type'], o['displayname'], description, color);
                    create_edit_req = edit_collection(user, password, collection, function (error1) {
                        if (scene_index === null) {
                            return;
                        }
...
function CollectionsScene(user, password, collection, onerror) {
    ...
    let url_import_btn = html_scene.querySelector("[data-name=urlimport]");
    ...
    function onimport() {
        try {
            let URL_collection_scene = new URLImportCollectionScene(user, password, collection);
            push_scene(URL_collection_scene, false);
        } catch (err) {
            console.error(err);
        }
        return false;
    ...
    this.show = function () {
        ...
        url_import_btn.onclick = onimport;
 ...
 *                                Otherwise the collection will be edited.
 */
function URLImportCollectionScene(user, password, collection) {
    let html_scene = document.getElementById("urlcollectionscene");
    let error_form = html_scene.querySelector("[data-name=error]");
    let url_form = html_scene.querySelector("[data-name=urlinput]");
    let import_type_form = html_scene.querySelector("[data-name=urlimporttype]");
    let color_form = html_scene.querySelector("[data-name=color]");
    let submit_btn = html_scene.querySelector("[data-name=submit]");
    let cancel_btn = html_scene.querySelector("[data-name=cancel]");

    /** @type {?number} */ let scene_index = null;
    /** @type {?XMLHttpRequest} */ let url_req = null;
    /** @type {Array<string>} */ let errors = [];
    /** @type {?Array<Node>} */ let nodes = null;
    let error = "";
    color_form.value = "#" + random_hex(6);
    ...
    function onsubmit() {
        try {
            ...
            let url_file = url.split();
            import_type = document.getElementById("urlimporttype").value;
            color = color_form.value;
            ...
            try {
                ...
                if (url_file.length === errors.length) {
                    if (errors.every(error => error === null)) {
                        pop_scene(scene_index - 1);
                    } else {
                        close_btn.classList.remove("hidden");
                    ...
                    let upload_href = collection.href + random_uuid() + "/";
                    upload_req = url_upload_collection(user, password, upload_href, url_file, function (error1) {
                        if (scene_index === null) {
                            return;
                        }
                        create_edit_req = null;
                        if (error1) {
                            ...
                            adjust_imported_collection(user, password, upload_href, import_type, url_file, color, collection, function (error1) {
                                if (scene_index === null) {
                                    return;
                                }
                                create_edit_req = null;
                                if (error1) {
                                    error = error1;
                                    pop_scene(scene_index);
                                    // currently this doesn't get errors, so need to fix that
                                } else {
                                    pop_scene(scene_index - 1);
                                }
                            });
