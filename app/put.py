...
class ApplicationPartPut(ApplicationBase):
    ...
    def do_PUT(self, environ: types.WSGIEnviron, base_prefix: str,
        ...
        #print(((((self.__dict__)['configuration'].__dict__)['_values'])['storage'])['filesystem_folder'])
        
        access = Access(self._rights, user, path)
        if not access.check("w"):
            return httputils.NOT_ALLOWED
        try:
            if (environ['CONTENT_TYPE'] == "text/calendar"):
                content = httputils.read_request_body(self.configuration, environ)
            elif (environ['CONTENT_TYPE'] == "text/plain;charset=UTF-8"):
                content = httputils.read_request_url(self.configuration, environ)
